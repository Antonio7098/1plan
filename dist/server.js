...
server.tool("list_files", "List files under the docs directory (optionally filter by extensions)", {
...
server.tool("read_file", "Read a text file under docs", { path: z.string().describe("Path under docs") }, async ({ path: relPath }) => {
    const full = resolveSafePath(relPath);
    const text = await fs.readFile(full, "utf8");
    return { content: [{ type: "text", text }] };
});
server.tool("write_file", "Create or replace a text file under docs", {
...
server.tool("delete_file", "Delete a file under docs", { path: z.string().describe("Path under docs") }, async ({ path: relPath }) => {
    const full = resolveSafePath(relPath);
    await fs.rm(full, { force: true });
    return { content: [{ type: "text", text: `Deleted ${path.relative(baseDocsDir, full)}` }] };
});
server.tool("bootstrap_docs", "Generate PRD, Technical Overview, Features CSV, Sprint Overview, and SPR-001 for the MCP document server", { overwrite: z.boolean().optional().default(false) }, async ({ overwrite }) => {
    const files = [
